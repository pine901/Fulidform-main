...
import { ROUTE_AUTH_STACK_NAVIGATOR, ROUTE_SIGNIN, ROUTE_RECIPE_DETAIL } from '../../../routes/RouteNames';
import { apiGetRecipeList, apiLoginRequired, apiResponseIsSuccess, API_PAGE_SIZE, } from '../../../utils/API';
...
export default RecipeListScreen = (props) => {
  ...
  const loadItemList = async (pageNumer = 1, search_payload = STATIC_VALUES.current['searchPayload'], force_reload = false) => {
    ...
    const response = await apiGetRecipeList(payload);
  ...
  const vewItemDetail = (itemId) => {
    navigation.navigate(ROUTE_RECIPE_DETAIL, { itemId: itemId })
  }
      ...
      <View style={[CustomStyle.flexibleSplitter, CustomStyle.itemListPageWrapper]}>
        <View style={[CustomStyle.searchBoxWrapper]}>
          <View style={CommonStyles.body}>
            <Text fontSize="lg">FIND A RECIPE</Text>
          </View>
          <RecipeSearchBox
            {...props}
