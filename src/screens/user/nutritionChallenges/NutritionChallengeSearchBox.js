...
import { apiGetNutritionChallengeTaxonomies, apiLoginRequired, apiResponseIsSuccess, } from '../../../utils/API';
...
export default NutritionChallengeSearchBox = (props) => {
  ...
  const loadFilterBoxData = async () => {
    const apiKey = "apiGetNutritionChallengeTaxonomies"
    if (checkApiIsLoading(apiKey)) {
      return false;
    }
    startApiLoading(apiKey);
    const response = await apiGetNutritionChallengeTaxonomies()
