...
import { addItemToArray, console_log, empty, equalTwoOjects, get_utc_timestamp_ms, removeItemFromArray } from '../../../utils/Misc';
import { ROUTE_AUTH_STACK_NAVIGATOR, ROUTE_SIGNIN, ROUTE_NUTRITION_CHALLENGE_DETAIL } from '../../../routes/RouteNames';
import { apiGetNutritionChallengeList, apiLoginRequired, apiResponseIsSuccess, API_PAGE_SIZE, } from '../../../utils/API';
import { signOut } from '../../../redux/auth/actions';
import { navReset } from '../../../utils/Nav';
import NutritionChallengeSearchBox from './NutritionChallengeSearchBox';
import { useRef } from 'react';
import NutritionChallengeItem from './NutritionChallengeItem';
...
export default NutritionChallengeListScreen = (props) => {
  ...
  const reloadItemList = async (search_payload = STATIC_VALUES.current['searchPayload']) => {
    const pageNumer = 1;
  ...
  const loadItemList = async (pageNumer = 1, search_payload = STATIC_VALUES.current['searchPayload'], force_reload = false) => {
    console_log("pageNumer:::", pageNumer)
    const apiKey = "apiGetNutritionChallengeList";
    if (!force_reload) {
      if (itemsLoading) {
        return false;
      }
    ...
    const response = await apiGetNutritionChallengeList(payload);
  ...
  const vewItemDetail = (itemId) => {
    navigation.navigate(ROUTE_NUTRITION_CHALLENGE_DETAIL, { itemId: itemId })
  }
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <View style={[CustomStyle.flexibleSplitter, CustomStyle.itemListPageWrapper]}>
        <View style={[CustomStyle.searchBoxWrapper]}>
          <View style={CommonStyles.body}>
            <Text fontSize="lg">FIND A MEAL PLAN</Text>
          </View>
          <NutritionChallengeSearchBox
            {...props}
